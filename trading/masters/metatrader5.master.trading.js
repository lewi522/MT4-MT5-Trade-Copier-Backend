...
const getMetatrader5OrderPair = async (callback) => {
  ...
  for (let i = 0; i < contractData.rowCount; i++) {
    ...
    if (contract.my_master_type === 'mt4') {
      const master = await client.query(
        `SELECT token 
          FROM accounts 
          WHERE account_id = $1
...
const calc_tp_st = (master_account_balance, contract, copier_account_balance, exist_order, pip_value) => {
  const risk_type = contract.risk_type;
  const risk_setting = contract.risk_setting;
  const follow_tp_st = contract.follow_tp_st;
  let stopLoss = 0;
...
const calc_volume = (fixed_master_acc_balance, master_account_balance, copier_account_balance, copier_lot_size, risk_type, risk_setting, history_order_lot, exist_order_lot) => {
  ...
  switch (risk_type) {
    case 'fixed_lot':
      volume = Math.round(copier_lot_size * (history_order_lot - exist_order_lot) / history_order_lot * 100) / 100;
      break;
    case 'balance_multiplier':
      volume = Math.round((risk_setting * (copier_account_balance * 100) / (master_account_balance * 100)) * (history_order_lot - exist_order_lot)) / 100;
      // volume = history_order_lot - exist_order_lot;
      break;
    case 'lot_multiplier':
      volume = risk_setting === 100 ? (history_order_lot - exist_order_lot) : Math.round((history_order_lot - exist_order_lot) * risk_setting) / 100;
      break;
    case 'fixed_balance_multiplier':
      volume = Math.round(copier_account_balance / fixed_master_acc_balance * risk_setting * (history_order_lot - exist_order_lot)) / 100;
      break;
  }
  return volume;
}
    ...
    await metatrader5Axios.get('/CheckConnect', {
      ...
      await metatrader5Axios.get('/AccountSummary', {
        ...
        if (summary.status !== 200) {
          console.log("metatrader5-master ----------> get Account Summary Request Error", master.account_id);
          return;
        }
        await client.query(
          `UPDATE accounts
                    ...
                    if (contract.status) {
                      if (exist_order && (exist_order.takeProfit !== history_order.takeProfit || exist_order.stopLoss !== history_order.stopLoss)) {
                        await metatrader4Axios.get(`/SymbolParams`, {
                          ...
                          if (info.statusText === "OK") {
                            const { stopLoss, takeProfit } = calc_tp_st(master.account_balance, contract, mt4_copier_account.rows[0].account_balance, exist_order, info.data.symbol.point);
                      ...
                      if (!exist_order || exist_order.lots !== history_order.lots) {
                        ...
                        if (exist_order) {
                          ...
                          volume = calc_volume(fixed_master_acc_balance, master.account_balance, mt4_copier_account.rows[0].account_balance, copier_order.lots, contract.risk_type, contract.risk_setting, history_order.lots, exist_order.lots);
                          if (volume === 0) return;
                        }
                  ...
                  if (pair) {
                    if (contract.status) {
                      ...
                      if (exist_order && (exist_order.takeProfit !== history_order.takeProfit || exist_order.stopLoss !== history_order.stopLoss)) {
                        await metatrader5Axios.get(`/SymbolParams`, {
                          ...
                          if (info.statusText === "OK") {
                            ...
                            const { stopLoss, takeProfit } = calc_tp_st(master.account_balance, contract, mt5_copier_account.rows[0].account_balance, exist_order, info.data.symbolInfo.points);
                              ...
                              if (modify_response.status === 200) {
                                ...
                                if (user.rowCount > 0 && user.rows[0].notification_setting?.modify_trade) {
                                  ...
                                  if (socketUsers[mt5_copier_account.rows[0].user_id]) {
                                    io.to(parseInt(mt5_copier_account.rows[0].user_id)).emit('notification', messages.rowCount > 0 ? messages.rows[0] : {});
                                  }
                                }
                              }
                            }).catch(() => {
                      ...
                      if (!exist_order || exist_order.lots !== history_order.lots) {
                        ...
                        if (exist_order) {
                          const fixed_master_acc_balance = contract.risk_type === "fixed_balance_multiplier" ? contract.fixed_master_acc_balance : master.account_balance;
                          volume = calc_volume(fixed_master_acc_balance, master.account_balance, mt5_copier_account.rows[0].account_balance, copier_order.lots, contract.risk_type, contract.risk_setting, history_order.lots, exist_order.lots);
                          if (volume === 0) return;
                        }
                        const real_lot_size = (copier_order && copier_order.lots <= volume) ? 0 : volume;
                      ...
                      if (info.statusText === "OK") {
                        ...
                        await metatrader4Axios.get('/OrderSend', {
                          ...
                          if (order_response.status === 200) {
                            ...
                            console.log(indexNum, "metatrader5-master ----------> metatrader4 order success", performance.now())
                          }
                        }).catch((error) => {
                          console.log("metatrader5-master ----------> metatrader4 order send error", error);
                        });
                      }
                    })
                  }
                    ...
                    await metatrader5Axios.get(`/SymbolParams`, {
                      ...
                      if (info.statusText === "OK") {
                        ...
                        await metatrader5Axios.get('/OrderSend', {
                          ...
                          console.log("metatrader5-master ----------> metatrader5 order send error", err);
                        });
                      }
                    })
                  }
